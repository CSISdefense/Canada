...
PrimeContractCount$MaxofCanadaRelevant<-TRUE
...
SubContractCount<-ddply(subset(SubContract2011,Fiscal.Year<=as.Date('2016-01-01')),
      .(MaxofCanadaRelevant),
      summarise,
      Obligation.2014=sum(Obligation.2014),
      ContractCount=length(CSIScontractID))
...
SubContractCount$IsSubcontractReportingContract<-1
...
PrimeAnnual<-ddply(PrimeAnnual,
      .(Fiscal.Year),
      transform,
      PercentObligations=Obligation.2014/sum(Obligation.2014))

PrimeAnnual<-subset(PrimeAnnual,IsSubcontractReportingContract==1,select=-c(Obligation.2014))
