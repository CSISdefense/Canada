...
TotalFSRSAnnual<-ddply(subset(TotalFSRS,Fiscal.Year<as.Date("2016-01-01")),
      .(Fiscal.Year),
      summarise,
      TotalSub.2014=sum(TotalSub.2014))

SubContractAnnualOnlySub<-plyr::join(SubContractAnnualOnlySub,TotalFSRSAnnual)
SubContractAnnualOnlySub$PercentCanada<-SubContractAnnualOnlySub$Obligation.2014/SubContractAnnualOnlySub$TotalSub.2014

SubContractAnnualAll<-plyr::join(SubContractAnnualAll,TotalFSRSAnnual)
SubContractAnnualAll$PercentCanada<-SubContractAnnualAll$Obligation.2014/SubContractAnnualAll$TotalSub.2014
SubContractAnnualAll$MaxofCanadaRelevant<-"All"
SubContractAnnual<-rbind(SubContractAnnualOnlySub,SubContractAnnualAll)
...
TotalFPDSannual<-ddply(subset(TotalFPDS,Fiscal.Year>=as.Date("2011-01-01")),
      .(Fiscal.Year),
      summarise,
      TotalPrime.2014=sum(TotalPrime.2014))

PrimeAnnual<-plyr::join(PrimeAnnual,TotalFPDSannual)
PrimeAnnual$PercentCanada<-PrimeAnnual$Obligation.2014/PrimeAnnual$TotalPrime.2014
...
ggplot(SubContractAnnual,
  ...
  geom_line(stat="identity")+ theme(legend.position="bottom",
              ...
              legend.title= element_text(size=8),
              legend.text= element_text(size=8))+
  scale_y_continuous("Percent of Obligations to Canada Related Vendors",labels = scales::percent)+
  scale_x_date("Subaward Report Year")+
